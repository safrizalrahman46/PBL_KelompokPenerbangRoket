...
class ReceiptService {
  Future<void> printReceipt({
    ...
    if (tableId == null || paymentMethod == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Pilih meja dan metode pembayaran dulu.')));
      return;
    }
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              final pdfData = await _generatePdfReceipt(cart, customerName, tableName, paymentMethod, received, change);
              await Printing.layoutPdf(onLayout: (PdfPageFormat format) async => pdfData);
              if (context.mounted) {
                Navigator.of(context).pop();
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nota berhasil dicetak!')));
              }
            } catch (e) {
              if (context.mounted) {
                Navigator.of(context).pop();
      ...
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              // Header
              pw.Center(child: pw.Text('EAT.O RESTAURANT', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold))),
              pw.SizedBox(height: 4),
              pw.Center(child: pw.Text('NOTA PEMBAYARAN', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold))),
              pw.Divider(height: 10, thickness: 1),
              
              // Customer Info
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text('Nama: $customerName', style: const pw.TextStyle(fontSize: 10)),
                pw.Text('Meja: $tableName', style: const pw.TextStyle(fontSize: 10)),
              ]),
              pw.SizedBox(height: 4),
              pw.Text('Tanggal: ${_getFormattedDate()}', style: const pw.TextStyle(fontSize: 10)),
              pw.Text('Waktu: ${_getFormattedTime()}', style: const pw.TextStyle(fontSize: 10)),
              ...
              pw.Row(children: [
                ...
                pw.Expanded(flex: 1, child: pw.Text('Qty', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold))),
                pw.Expanded(flex: 2, child: pw.Text('Harga', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold))),
              ]),
              ...
              for (final item in cart.items)
                pw.Padding(
                  ...
                  child: pw.Row(children: [
                    pw.Expanded(flex: 2, child: pw.Text(item.menu.name, style: const pw.TextStyle(fontSize: 9), maxLines: 2)),
                    pw.Expanded(flex: 1, child: pw.Text('${item.quantity}', style: const pw.TextStyle(fontSize: 9))),
                    pw.Expanded(flex: 2, child: pw.Text('Rp ${(item.menu.price * item.quantity).toStringAsFixed(0)}', style: const pw.TextStyle(fontSize: 9))),
                  ]),
                ),
              ...
              _buildPdfSummaryRow('Pajak 10%', 'Rp ${(cart.subtotal * cart.taxPercent / 100).toStringAsFixed(0)}'),
              ...
              pw.Center(child: pw.Column(children: [
                pw.Text('Terima kasih atas kunjungannya!', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 4),
                pw.Text('Semoga makanan kami memuaskan :)', style: const pw.TextStyle(fontSize: 8)),
              ])),
            ],
          );
        },
